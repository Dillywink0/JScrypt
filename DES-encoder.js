...
function initialPermutation(input) {
    return permute(input, initialPermutationTable);
}

// Perform the final permutation
function finalPermutation(input) {
    return permute(input, finalPermutationTable);
}

// Perform a permutation according to a given permutation table
...
function expansionPermutation(input) {
    return permute(input, expansionPermutationTable);
}
...
function substitute(input) {
    let output = '';
    for (let i = 0; i < 8; i++) {
        const sBoxIndex = parseInt(input.substring(i * 6, (i + 1) * 6), 2);
        const row = parseInt(input[i * 6] + input[i * 6 + 5], 2);
        const col = parseInt(input.substring(i * 6 + 1, (i + 1) * 6 - 1), 2);
...
function generateKeySchedule(key) {
    const keySchedule = [];
    let shiftedKey = key.substring(1) + key[0];
    for (let i = 0; i < 16; i++) {
        keySchedule.push(permute(shiftedKey, keySchedulePermutation));
        shiftedKey = shiftedKey.substring(1) + shiftedKey[0];
    }
    return keySchedule;
}
...
function desRound(input, key) {
    const expanded = expansionPermutation(input);
    const xor = xor(expanded, key);
    const substituted = substitute(xor);
    return substituted;
}
...
function desEncrypt(plainText, key) {
    // Perform the initial permutation
    let output = initialPermutation(plainText);
    // Generate the key schedule
    const keySchedule = generateKeySchedule(key);
    // Perform 16 rounds of DES
    for (let i = 0; i < 16; i++) {
        output = desRound(output, keySchedule[i]);
    }
    // Perform the final permutation
...
function desDecrypt(cipherText, key) {
    // Perform the initial permutation
    let output = initialPermutation(cipherText);
    // Generate the key schedule
    const keySchedule = generateKeySchedule(key);
...
// Example usage
const plainText = '011100110111000001100001011100100110010001100101';
const key = '1010101010101010101010101010101010101010101010101010101010101010';
